...
2. **Common header for all endpoints (except GET /api/calls and GET /api/calls/[id]):**
   ```
   x-api-key: {{api_key}}
   ```
...
**Note:** The `call_id` field is optional. If not provided, a unique ID will be generated automatically by the server.

**Body (raw JSON) - Deal accepted (with call_id):**
```json
{
  "call_id": "call_001",
...
**Body (raw JSON) - Negotiation failed:**
```json
{
  "call_id": "call_003",
  "started_at": "2025-12-15T12:00:00Z",
  "transcript": "Agent: Hello, thank you for calling. Can I get your MC number? Carrier: 456789. Agent: Verified! I have a load available. After 3 rounds of negotiation, we couldn't reach an agreement.",
  "outcome": "negotiation_failed",
  "sentiment": "neutral",
  "mc_number": "456789",
  "selected_load_id": "LD-1024",
  "initial_rate": 3500,
  "final_rate": null,
...
- `outcome`: `booked_transfer`, `not_verified`, `no_load_found`, `negotiation_failed`, `not_interested`, `call_dropped`
- `sentiment`: `positive`, `neutral`, `negative`
- `dateFrom`: `2025-12-15T00:00:00Z` (ISO datetime format)
- `dateTo`: `2025-12-16T23:59:59Z` (ISO datetime format)
- `search`: Search term (searches in transcript, mc_number, selected_load_id)
- `page`: Page number (default: 1)
- `limit`: Results per page (default: 20, max: 100)

**Example URLs:**
- No filters: `{{base_url}}/api/calls`
- With outcome filter: `{{base_url}}/api/calls?outcome=booked_transfer`
- With sentiment filter: `{{base_url}}/api/calls?sentiment=positive`
- With multiple filters: `{{base_url}}/api/calls?outcome=booked_transfer&sentiment=positive`
- With date range: `{{base_url}}/api/calls?dateFrom=2025-12-15T00:00:00Z&dateTo=2025-12-16T23:59:59Z`
...
1. **Create some example calls:**
   - Execute POST `/api/calls` with the examples above (call_001, call_002, call_003, call_004)
...
5. **Test filters on dashboard:**
   - GET `/api/calls?outcome=booked_transfer`
   - GET `/api/calls?sentiment=positive`
