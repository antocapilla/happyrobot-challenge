...
**Body (raw JSON) - Search by equipment:**
```json
{
...
3. **Test load search:**
   - POST `/api/loads/search` with different criteria
...
**401 Unauthorized:**
- Verify that the `Authorization: Bearer {{api_key}}` header is present
